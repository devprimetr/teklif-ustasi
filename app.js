...
function kaydet() {
  try {
    localStorage.setItem(DEPO, JSON.stringify({
      sektor: durum.sektor, fiyatlar: durum.fiyatlar, tema: durum.tema
    }));
...
function hesapla() {
  ...
  for (const k of s.kalemler) {
    if (k.tip === "birim") {
      ...
      satirlar.push({ ad: k.ad, detay: `${m} ${k.birim} × ${para(f)}`, tutar });
      ara += tutar;
    } else if (k.tip === "opsiyon") {
      if (!durum.opsiyonlar[k.kod]) continue;
      const tutar = fiyatAl(k);
      satirlar.push({ ad: k.ad, detay: "", tutar });
      ara += tutar;
    }
  }
  ...
  for (const k of s.kalemler) {
    ...
    satirlar.push({ ad: k.ad, detay: `%${o}`, tutar });
    ara += tutar;
  }
  ...
  $("#kalemler").innerHTML = s.kalemler.map(k => {
    ...
    if (k.tip === "birim") {
      return `<div class="satir">
        <div class="bilgi"><div class="ad">${k.ad}</div>
...
$("#fiyatDuzenle").onclick = () => {
  ...
  $("#fiyatAlanlari").innerHTML = s.kalemler.map(k => `
    <div style="margin-bottom:12px">
      <label>${k.ad}${k.tip === "yuzde" ? " (%)" : k.birim ? ` (₺/${k.birim})` : " (₺)"}</label>
      <input type="number" inputmode="decimal" step="any" data-fk="${k.kod}"
             value="${fiyatAl(k)}"></div>`).join("");
  $("#fiyatDlg").showModal();
};
...
<title>Teklif${musteri ? " — " + musteri : ""}</title><style>
...
<header><div><div class="buyuk">${s.ikon} ${s.ad}</div>
<div style="color:#666;font-size:13px">Fiyat Teklifi</div></div>
...
${musteri ? `<p style="margin:0 0 20px"><span style="color:#888;font-size:11px;
  ...
  <span style="font-size:16px;font-weight:600">${musteri}</span></p>` : ""}
<table>${h.satirlar.map(r => `<tr><td><div class="ad">${r.ad}</div>
${r.detay ? `<div class="detay">${r.detay}</div>` : ""}</td>
...
<div class="kosul"><h2>Koşullar</h2><ul>${s.kosullar.map(k => `<li>${k}</li>`).join("")}</ul></div>
